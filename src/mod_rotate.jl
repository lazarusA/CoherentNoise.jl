...
"""
function rotate(source::S; x::Real=0.0, y::Real=0.0, z::Real=0.0) where {N,S<:AbstractSampler{N}}
    cx, cy, cz = cos.((x, y, z))
    sx, sy, sz = sin.((x, y, z))
    rx = (sx * sy * sz + cy * cz, cx * sz, sy * cz - cy * sx * sz)
    ry = (sx * sy * cz - cy * sz, cx * cz, -cy * sx * cz - sy * sz)
    rz = (-sy * cx, sx, cy * cx)
    Rotate{N,S}(random_state(source), source, rx, ry, rz)
end

@inline function sample(
    sampler::Rotate{2,S},
    coords::Vararg{Real,2},
) where {S<:AbstractSampler{2}}
...
@inline function sample(
    sampler::Rotate{3,S},
    coords::Vararg{Real,3},
) where {S<:AbstractSampler{3}}
...
@inline function sample(
    sampler::Rotate{4,S},
    coords::Vararg{Real,4},
) where {S<:AbstractSampler{4}}
